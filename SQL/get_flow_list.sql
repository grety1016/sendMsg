...
BEGIN 
	...
	FROM V_WF_ASSIGN a WITH(NOLOCK)
	INNER JOIN T_SEC_USER u WITH(NOLOCK) ON a.FReceiverId=u.FUSERID
	INNER JOIN (SELECT DISTINCT flownumber FROM SendMessage WITH(NOLOCK)) sg on a.fnumber = sg.flownumber --当流程实例已经在发送列表中出现多次，仅取出其中一条来匹配，按时间戳来判断是否要加入到发送列表
	WHERE a.fcreatetime  NOT IN (SELECT fcreatetime FROM	dbo.SendMessage sg2 WITH(nolock))
